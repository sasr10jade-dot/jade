...
- [x] 연속 듣기 — 전역 미니플레이어 + 재생 큐. Discover/Home의 트랙 카드에 재생 버튼을
      추가해, 클릭하면 그 목록(피처드/인기급상승/좋아요많은/장르별 행, 홈의 각 행)
      순서 그대로 큐가 만들어지고 한 곡이 끝나면 다음 곡으로 자동 전환된다.
  - `QueueProvider`(`src/components/player/queue-context.tsx`)를 레이아웃 최상단에
    한 번만 마운트해서, 페이지를 이동해도(App Router 클라이언트 사이드 전환) `<audio>`
    엘리먼트가 언마운트되지 않고 재생이 끊기지 않음. 화면 하단 고정 바(`queue-bar.tsx`)
    로 재생/일시정지/이전/다음/탐색/큐 진행률(N/M) 표시.
  - 트랙 상세 페이지의 기존 개별 플레이어(가이드 A/B 비교 등)와 동시에 소리가 겹치지
    않도록, 어느 쪽이든 재생을 시작하기 직전에 다른 쪽을 멈추는 최소한의 전역 조정
    장치 추가(`src/lib/now-playing.ts` — claimPlayback).
  - 범위: Discover/Home은 목록 전체가 큐로 이어지고, Likes/프로필 등 다른 페이지의
    트랙 카드는 재생 버튼은 동일하게 동작하되(단일 곡 재생) 목록 전체를 잇지는 않음 —
    "쭉 훑어 들으며 넘어가는" 흐름이 실제로 일어나는 두 곳에 우선 적용.

- [x] 이용 가이드 + 고객센터 — 그동안 없었던 온보딩/지원 경로를 채움.
  - **이용 가이드**(`/guide`) — 역할별(Creator/Performer/Buyer) 단계별 사용법 +
    자주 묻는 질문(에스크로, VOICE Cash, 라이선스 종류, 곡 의뢰, Split, 가격 제안 만료
    등 이번 세션에서 쌓인 개념들 정리).
  - **고객센터**(`/support`) — 실제 문의 티켓 시스템. 사용자가 문의 등록 →
    `/admin/support`에서 관리자가 확인·답변(같은 스레드 페이지를 사용자/관리자
    양쪽이 공유해서 보되, 답변 등록·문의 종료 버튼만 관리자에게 추가로 노출) →
    답변이 달리면 알림(`SUPPORT_TICKET_REPLIED`). 사용자가 답변/종료된 문의에 다시
    답장하면 `OPEN`으로 재오픈되어 관리자 큐에 다시 노출됨. 관리자 대시보드에도
    "미답변 고객문의" 카운트 카드 추가.
  - `/legal`도 지금까지 어디서도 링크되지 않던 페이지였음 — 세 페이지 모두 모든
    페이지 하단의 새 푸터(`site-footer.tsx`)에서 진입 가능하도록 함(헤더 내비게이션은
    이미 항목이 많아 자리를 만들지 않음).

- [x] 큐 플레이어에 랜덤재생/반복재생 추가 — 큐 내부 상태를 `order`(큐 인덱스의 순열) +
      `position`(그 순열 안에서의 위치)로 재구성. 셔플을 켜면 order만 새로 섞되 지금
      재생 중인 곡은 맨 앞에 고정해 갑자기 곡이 바뀌지 않게 하고, 끄면 원래 순서로
      복귀. 반복을 켜면 큐 끝(또는 처음)에서 next()/prev()가 멈추지 않고 순환.

- [x] 큐 플레이어 전체화면("크게 보기") — 하단 바의 곡 정보나 확장 아이콘을 누르면
      전체화면 오버레이(`QueueExpandedView`)로 전환. 큰 앨범아트/컨트롤 아래에 현재
      재생목록 전체(`orderedQueue` — order 순서대로 정렬된 큐)를 나열해 원하는 곡을
      바로 클릭해 재생(`playAt(position)`)할 수 있음. 확장/축소는 같은 재생 상태를
      공유하므로 전환해도 끊기지 않음.

- [x] 큐 플레이어 전체화면에 오디오 비주얼라이저 추가 — 트랙 상세 페이지 플레이어가
      쓰는 것과 동일한 전역 EQ/분석 그래프(`audioEngine`)에 큐의 `<audio>` 엘리먼트도
      연결(`audioEngine.connect()` + `resume()`, 재생 성공 이후 별도 시도 — 기본 재생은
      절대 막히지 않는 기존 원칙 유지)해서, 헤더의 이퀄라이저 설정이 큐 재생에도 그대로
      적용되고 같은 `AudioVisualizer` 컴포넌트를 재사용할 수 있게 됨.

- [x] 태그 검색 활용 — 업로드 때만 입력받고 어디서도 안 쓰이던 `Track.tags`를 실제로
      연결. `/search`가 제목/장르/무드/크리에이터에 이어 태그도 매칭하고, `/discover`에
      "태그: 전체" 드롭다운 필터 추가(콤마로 이어붙인 자유 문자열이라 distinct 조회가
      안 돼 전체를 가져와 쪼개고 중복 제거해서 옵션 목록을 만듦).
- [x] 트랙 비공개 전환 — Studio에 "비공개로 전환" 버튼. 판매 이력이 걸린 트랙은
      하드 삭제 대신 숨김만 지원(`Track.removedByCreator`, 관리자용 `removedByAdmin`과
      같은 패턴). Discover/홈/검색/좋아요/매칭/프로필 등 노출 지면 12곳 전부에
      `removedByAdmin: false, removedByCreator: false`를 동시에 걸어야 해서 각
      페이지의 조회 조건을 전부 맞춰 수정. 이미 구매한 사람의 다운로드 링크·트랙 상세
      직접 접속은 기존 admin 숨김과 동일하게 계속 열려 있음(발견 지면에서만 숨김).

- [x] 관리자 화면 확장 (3단계) — 기존 관리자 화면을 유지하면서 트랙 100건+ 규모에서
      실사용 가능하도록 보완.
  1. **사용자/트랙 검색·필터** — `/admin/users`(이름/이메일 검색, 역할·정지 필터),
     `/admin/tracks`(제목/크리에이터 검색, 숨김 상태 필터) 둘 다 이전엔 전체 목록만
     쭉 나열해서 트랙 100건 넘어가면 Ctrl+F 말곤 방법이 없었음.
  2. **트랙 모더레이션에 숨김 주체 구분** — "관리자 숨김"/"크리에이터 비공개" 배지를
     분리해서, 관리자가 숨긴 건지 크리에이터 본인이 비공개 전환한 건지 구분되게 함.
  3. **곡 의뢰 관리 화면 신설**(`/admin/commissions`) — 등록→지원→매칭→납품 전체
     흐름이 관리자 쪽엔 아예 노출되는 곳이 없었음. 상태별 정렬 + 검색/필터, 선정된
     크리에이터·납품곡을 한 줄에 요약 표시. 실제 조치(취소/납품)는 당사자가 하는
     영역이라 읽기 전용 오버사이트로 설계.
